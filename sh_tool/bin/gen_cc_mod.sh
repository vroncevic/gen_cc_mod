...
declare -A GEN_CC_MOD_LOGO_DATA=(
    [OWNER]="vroncevic"
    [REPO]="${GEN_CC_MOD_TOOL}"
    [VERSION]="${GEN_CC_MOD_VERSION}"
    [LOGO]="${GEN_CC_MOD_LOGO}"
)
...
function __gen_cc_mod {
    local MN=$1
    if [ -n "${MN}" ]; then
        display_logo GEN_CC_MOD_LOGO_DATA
